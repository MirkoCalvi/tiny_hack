...
enum Cat : uint8_t { PLASTIC=0, PAPER=1, GLASS=2, ORGANIC=3, GENERAL=4 };

// Map to 5 distinct discrete colors (no PWM)
static void showCategory(uint8_t cat) {
  switch (cat) {
    case PLASTIC: setRGBDiscrete(true,  true,  false); break; // yellow  (R+G)
    case PAPER:   setRGBDiscrete(false, false, true ); break; // blue    (B)
    case GLASS:   setRGBDiscrete(false, true,  false); break; // green   (G)
    case ORGANIC: setRGBDiscrete(true,  false, false); break; // red     (R) ~ brown-ish
    case GENERAL: setRGBDiscrete(true,  true,  true ); break; // white   (R+G+B) ~ grey
