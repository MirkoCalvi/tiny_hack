...
// ================== PERSON labels ==================
static const char* PERSON_CLASSES[CLASSES] = {
  "person","not_person"
};
// ================== Helpers colore ==================
...
static void sigmoidf(const float* in, int n, float* out) {
    for (int i = 0; i < n; ++i) {
        out[i] = 1 / (1 + powf(EULER_NUMBER_F, -in[i]));
    }
}
...
void loop(){
  ...
  float p2[CLASSES];
  softmax_vec(out_raw, (int)CLASSES, p2);
  // sigmoidf(out_raw, (int)CLASSES, p2);
  Serial.print("[ZANT] Stampa softmax: ");
  Serial.println();
  ...
  if (streaming()) {
    ...
    // log testuale locale
    const char* name = (cls>=0 && cls<(int)CLASSES) ? PERSON_CLASSES[cls] : "unknown";
    Serial.print("[ZANT] PERSON top1: idx=");
    Serial.print(cls);
    Serial.print(" label=");
    Serial.print(name);
    Serial.print(" prob=");
