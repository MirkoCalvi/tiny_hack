...
static const uint32_t CLASSES= 5;               // COCO80
    ...
    float* __restrict dst_f, uint8_t* __restrict dst_gray)
{
  ...
  for (int y = 0; y < (int)H; ++y) {
    ...
    for (int x = 0; x < (int)W; ++x) {
      ...
      float val = (float)r * inv255;
      sumR  += val;
      sumR2 += val * val;
    }
  }

  float avg = (float)(sumR / numPixels);
  float std = sqrtf((float)(sumR2 / numPixels - avg * avg));
  ...
  for (int y = 0; y < (int)H; ++y) {
    ...
    for (int x = 0; x < (int)W; ++x) {
      ...
      const int di = y*W + x;
      dstR[di] = ((float)r * inv255 - avg)/std;
      dstG[di] = ((float)g * inv255 - avg)/std;
      dstB[di] = ((float)b * inv255 - avg)/std;
...
void loop(){
  ...
  int rc = predict(gInput, inputShape, 4, &out_raw);   // OUT: [80]
  ...
  int cls = 0; float prob = 0.f;
  top1(p80, (int)CLASSES, &cls, &prob);
  if (prob < 0.f) prob = 0.f; if (prob > 1.f) prob = 1.f;
  uint16_t prob_x1000 = (uint16_t) lrintf(prob * 1000.0f);

  // LED feedback locale
  digitalWrite(LED_BUILTIN, (prob >= THR) ? HIGH : LOW);

  if (streaming()) {
    // invia frame 96x96 gray + header FRME + CRC
    send_frame_gray_FRME(W, H, (uint8_t)cls, prob_x1000, ms_x10, gGray8);
  } else {
    // log testuale locale
    const char* name = (cls>=0 && cls<(int)CLASSES) ? COCO80[cls] : "unknown";
    Serial.print("[PROB]");
    for (int i = 0; i < CLASSES ; i++) {
      Serial.print(p80[i], 3);
      Serial.print(", ");
    }
  }

  // If prob is over the THR, a frame is sent
  if (prob >= THR) {
    Serial.println("\nFRAME 320x240:");
    Serial.write((uint8_t*)buf, 320 * 240 * 2);
    Serial.println("\nEND");
  }
